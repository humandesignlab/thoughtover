...
- **Inner-voice treatment** (`NARRATION_INNER_VOICE`) — proximity/ASMR-ish chain on each line: chest warmth, strong de-airing (reads internal not "in the room"), gentle compression (whispers survive), 70 ms fade-in so thoughts emerge softly. `NARRATION_INNER_VOICE_REVERB` (0..1) adds a short mental slap; `0` = dry bone-conduction, `0.25–0.35` = inside-the-head space.
- **Mix level** — `NARRATION_GAIN_DB` negative sits the voice *into* the mix; `NARRATION_DUCK_DB` deeper + `NARRATION_DUCK_FADE` longer isolates each thought from the trail without killing ambiance between lines.
- **Ambience as content, not a music bed** — the trail audio *is* the ride; it only gets a light high-pass, a **timed midrange carve** (voice pocket while a line plays), and ducking — **not** the inner-voice EQ/reverb/compression chain. That chain runs on ElevenLabs lines only.
